...
def parse_dataset(_dataset, _outdir, _max=100):
    """
...
def parse_dataset_zipped(path_zipped_json, _outdir, max_files=100):
    fnames_urls = []

    with zipfile.ZipFile(path_zipped_json, "r") as z:
    ...
    return fnames_urls[:max_files]

ROOT_PATH = r"/home/batman/Dropbox/DATA/iMaterialist Fashion"
dataset =  os.path.join(ROOT_PATH,"train.json.zip")
testset =  os.path.join(ROOT_PATH,"test.json.zip")
outdir = os.path.join(ROOT_PATH,"OUT")

fnames_urls = parse_dataset_zipped(dataset, outdir, max_files=5000)
...
#fnames_urls = parse_dataset(testset, outdir)
